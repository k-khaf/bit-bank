    ...
    mainPanel(
      ...
      fluidRow(
        ...
        conditionalPanel(
          ...
          helpText("This version of the app doesn't optimize over the whole index - just a representative example"),
...
server <- function(input, output, session) {
  ...
  output$table_dd_benchmark <- renderTable(rownames = TRUE, striped = TRUE, {
    ...
    output$textIndex <- renderText({
      if (length(colnames(opt.port1$rets_df1)) == 3 || length(colnames(opt.port$rets_df)) == 3){
      "Drawdowns for Index:"
      }
    })
