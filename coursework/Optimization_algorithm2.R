...
tickers <- c("FB", "AAPL", "AMZN", "NFLX", "GOOGL", "AMD", "NVDA","ARNC","CPRT","TGT")
...
#portfolioReturns <- readRDS("data/portfolioReturns.rds")



# Portfolio Setup
port3 <- portfolio.spec(colnames(portfolioReturns))
port3 <- add.constraint(port3, type = "box", min = 0.1, max = 0.4) # Box
...
# port3 <- add.constraint(port3, type = "position_limit", max_pos = 30) # no. non-0 variables = 30
...
#port.opt3 <- optimize.portfolio(portfolioReturns, port3, optimize_method = "random", trace = TRUE, rp = rportfolios3)
